...
maxFeat = max(pointData.pts,[],2);
minFeat = min(pointData.pts,[],2);
pointData.features = [bsxfun(@rdivide, bsxfun(@minus, pointData.pts, minFeat), maxFeat - minFeat); ones(1,pointData.numPts)];
...
for i=1:n
    ...
    [assigned_i,correct_i,percentClass_i,confusion_i,learned_i]= ...
        SVM(pointData,partitionSet{i},{0.0001, 1, 1});
...
for p=1:numPartitions
    ...
    title(sprintf('Partition %d, Training data (%d pt)',p,partitionSet{p}.trainSize));
